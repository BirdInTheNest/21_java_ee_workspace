...
$(function(){
	...
	$('#writeBtn').click(function(){
		...
		//name 속성
		if($('input[name="name"]').val() == '') { //input 태그 안에 name 속성
			$('#nameDiv').html('이름 입력');
...
function checkId(){
	...
	if(id == "")
		...
		window.open("/memberServlet/CheckIdServlet?id="+id, "checkId", 
		"width=300 height=100 top=200 left=700"); //타겟명을 주면 창 하나만 열림
}
...
writeForm.html	------------------------------> CheckIdServlet.java
                        get방식, 변수 = 값
아이디 중복체크                                       1. 데이터
                                                 2. DB
                                                 3. 응답
...
-------------------------------------------------------------------------
         서버명(Tomcat)/Context명
http://localhost:8080/memberServlet/CheckIdServlet?변수=값
http://localhost:8080/memberServlet/CheckIdServlet?id=document.getElementById("id").value
